...
def snake_draft(players):
    ...
    team_a, team_b = [], []
    # Reset index to ensure smooth iteration
    ...
    # Return DataFrames (handling empty lists to avoid crashes)
    if not team_a:
...
def format_team_list(df, team_name):
    """Formats the list of players for email (Names & Positions only)."""
    if df.empty:
        return f"{team_name} (0 players):\n"
    txt = f"{team_name} ({len(df)} players):\n"
    # Sort by Position for email clarity
...
# File Uploader (Updated for Excel)
uploaded_file = st.file_uploader("Upload Excel File", type=['xlsx', 'xls'])

if uploaded_file is not None:
    try:
        # Load Data (Updated for Excel)
        # Note: header=1 implies the first row is empty/title and the second row contains headers
        ...
        # Create Full Name immediately for easier tracking
        ...
        if available.empty:
            st.error("No players marked as 'Yes' in Availability column.")
            st.stop()
        ...
        BASE_F = 12
        BASE_D = 8
        MIN_D_CRITICAL = 6 # Absolute floor (3 per team) to play a viable game

        # Calculate Targets based on Total Player Count
        if total_players <= 20:
            target_f = BASE_F
            target_d = BASE_D
        else:
            extras = total_players - 20
            # First 6 extras go to Forwards (Max 18)
            add_to_f = min(extras, 6)
            extras -= add_to_f
            # Remaining extras go to Defense (Max 12)
            add_to_d = min(extras, 4)
            
            target_f = BASE_F + add_to_f
            target_d = BASE_D + add_to_d
        
        st.info(f"**Roster Strategy:** Found {total_players} players. Aiming for **{target_f} Forwards** and **{target_d} Defensemen**.")

        # --- 4. RANDOMIZE & SORT ---
        # Shuffle for randomness
        available = available.sample(frac=1).reset_index(drop=True)
        # Rank: Regular=0, Spare=1
        available['Status_Rank'] = available['Reg/Spare'].apply(lambda x: 0 if str(x).strip().upper() == 'R' else 1)
        # Sort: Regulars First, then Score
        ...
        # --- 5. FILL GAPS (DYNAMIC LOGIC) ---
        
        # STEP 0: CRITICAL DEFENSE CHECK (Ensure 3 per team minimum)
        ...
        # STEP 1: FILL FORWARDS (Reach dynamic target_f)
        if len(pool_f) < target_f:
            needed = target_f - len(pool_f)
            surplus_d = len(pool_d) - MIN_D_CRITICAL # Don't rob D below critical level
            ...
            if surplus_d > 0:
                ...
                if not candidates.empty:
                    ...
                    moved_names = ", ".join(converts['Full Name'].tolist())
                    st.info(f"Moved {len(converts)} player(s) from D to F to reach target: **{moved_names}**")

        # STEP 2: FILL DEFENSE (Reach dynamic target_d)
        # Only if F exceeds its specific target
        if len(pool_d) < target_d:
            ...
            if surplus_f > 0:
                ...
                if not candidates.empty:
                    ...
                    pool_f = pool_f.drop(converts.index)
                    moved_names = ", ".join(converts['Full Name'].tolist())
                    st.info(f"Moved {len(converts)} player(s) from F to D (using surplus F): **{moved_names}**")

        # --- 6. APPLY LIMITS & IDENTIFY CUTS ---
        # Sort pools again to ensure cuts are correct (Spares/Low score cut first)
        ...
        # Select the Team (Using Dynamic Targets)
        ...
        with col1:
            ...
            if not team_a.empty:
                st.dataframe(team_a[cols], hide_index=True)
            else:
                st.write("No players.")
        ...
        with col2:
            ...
            if not team_b.empty:
                st.dataframe(team_b[cols], hide_index=True)
            else:
                st.write("No players.")

        # --- 10. DISPLAY CUTS (ADMIN VIEW ONLY) ---
        if not cuts_d.empty or not cuts_f.empty:
            ...
            with c1:
                if not cuts_d.empty:
                    ...
                    for name in cuts_d['Full Name']:
                        st.write(f"- {name}")
                else:
                    st.success("No Defense cuts.")
                    
            with c2:
                if not cuts_f.empty:
                    st.error(f"**Forward Cuts ({len(cuts_f)}):**")
                    for name in cuts_f['Full Name']:
                        st.write(f"- {name}")
                else:
                    st.success("No Forward cuts.")
...
"""
            ...
            # 1. Create Standard Mailto Link (Default App)
            ...
            # 2. Create Gmail Web Link
            gmail_url = f"https://mail.google.com/mail/?view=cm&fs=1&bcc={safe_bcc}&su={safe_subject}&body={safe_body}"

            if len(recipients) > 0:
                col_email1, col_email2 = st.columns(2)
                with col_email1:
                    st.link_button("🚀 Open Default Email App", mailto_url)
                with col_email2:
                    st.link_button("📧 Draft in Gmail (Web)", gmail_url)
            else:
                st.caption("No emails found to generate link.")
